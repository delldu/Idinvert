...
def parse_args():
    ...
    parser.add_argument(
        "target_list", type=str, help="List of target images to diffuse from."
    )
    parser.add_argument(
        "context_list", type=str, help="List of context images to diffuse to."
    )
    parser.add_argument(
        "-o",
        "--output_dir",
    ...
    parser.add_argument(
        "-s", "--crop_size", type=int, default=110, help="Crop size. (default: 110)"
    )
    parser.add_argument(
        "-x",
        "--center_x",
...
def main():
    ...
    headers = ["Target Image", "Context Image", "Stitched Image", "Encoder Output"]
    ...
    for target_idx in tqdm(range(num_targets), desc="Target ID", leave=False):
        ...
        target_image = resize_image(
            load_image(target_list[target_idx]), (image_size, image_size)
        )
        visualizer.set_cell(target_idx * num_contexts, 0, image=target_image)
        for context_batch_idx in tqdm(
            range(0, num_contexts, args.batch_size), desc="Context ID", leave=False
        ):
            context_images = []
            for it in range(args.batch_size):
                ...
                context_image = resize_image(
                    load_image(context_list[context_idx]), (image_size, image_size)
                )
                context_images.append(context_image)
                visualizer.set_cell(row_idx, 1, image=context_image)
            code, viz_results = inverter.easy_diffuse(
                target=target_image,
    ...
    np.save(
        f"{output_dir}/{job_name}_inverted_codes.npy",
