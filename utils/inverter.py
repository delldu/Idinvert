...
class StyleGANInverter(object):
    ...
    def get_init_code(self, image):
        ...
        z = get_tensor_value(self.E.net(x).view(1, *self.encode_dim))
    ...
    def invert(self, image, num_viz=0):
        ...
        viz_results = []
        viz_results.append(self.G.postprocess(get_tensor_value(x))[0])
        x_init_inv = self.G.net.synthesis(z)
        viz_results.append(self.G.postprocess(get_tensor_value(x_init_inv))[0])
        ...
        for step in pbar:
            ...
            # Perceptual loss.
            # self.loss_feat_weight == perceptual_loss_weight -- 5e-5
            if self.loss_feat_weight:
                x_feat = self.F.net(x)
                x_rec_feat = self.F.net(x_rec)
                loss_feat = torch.mean((x_feat - x_rec_feat) ** 2)
                loss = loss + loss_feat * self.loss_feat_weight
                log_message += f", loss_feat: {get_tensor_value(loss_feat):.3f}"
            ...
            if num_viz > 0 and step % (self.iteration // num_viz) == 0:
                viz_results.append(self.G.postprocess(get_tensor_value(x_rec))[0])

            # len(viz_results) -- ...
            # (Pdb) viz_results[0].shape -- (256, 256, 3)
        return get_tensor_value(z), viz_results
    ...
    def diffuse(self, target, context, center_x, center_y, crop_x, crop_y, num_viz=0):
        ...
        init_z = get_tensor_value(self.E.net(x).view(-1, *self.encode_dim))
        ...
        copy_and_paste = self.G.postprocess(get_tensor_value(x))
        x_init_inv = self.G.net.synthesis(z)
        encoder_out = self.G.postprocess(get_tensor_value(x_init_inv))
        viz_results = {}
        ...
        for step in pbar:
            ...
            loss = loss + loss_pix * self.loss_pix_weight
            log_message = f"loss_pix: {np.mean(get_tensor_value(loss_pix)):.3f}"
            ...
            if self.loss_feat_weight:
                ...
                loss = loss + loss_feat * self.loss_feat_weight
                log_message += (
                    f", loss_feat: {np.mean(get_tensor_value(loss_feat)):.3f}"
                )

            log_message += f", loss: {np.mean(get_tensor_value(loss)):.3f}"
            pbar.set_description_str(log_message)
            if self.logger:
                self.logger.debug(
                    f"Step: {step:05d}, "
            ...
            if num_viz > 0 and step % (self.iteration // num_viz) == 0:
                rec_res = self.G.postprocess(get_tensor_value(x_rec))
