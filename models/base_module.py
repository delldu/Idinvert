...
class BaseModule(object):
    ...
    def __init__(self, model_name, module_name, logger=None):
        ...
        # Get paths.
        self.weight_path = model_settings.get_weight_path(f"{model_name}_{module_name}")

        # Build graph and load pre-trained weights.
        self.logger.info(
            f"Build network for module `{self.module_name}` in "
    ...
    def check_attr(self, attr_name):
        ...
        if not hasattr(self, attr_name):
            raise AttributeError(
                f"Field `{attr_name}` is missing for "
    ...
    def batch_run(self, inputs, run_fn):
        ...
        if inputs.shape[0] > self.ram_size:
            self.logger.warning(
                f"Number of inputs on RAM is larger than "
            ...
            if isinstance(batch_outputs, dict):
                for key, val in batch_outputs.items():
                    if not isinstance(val, np.ndarray):
                        raise ValueError(
                            f"Each item of the model output should be with "
                ...
                raise ValueError(
                    f"The model output can only be with type "
