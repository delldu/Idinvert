...
def parse_args():
    ...
    parser.add_argument(
        "--step", type=int, default=7, help="Number of manipulation steps. (default: 7)"
    )
    parser.add_argument(
        "--start_distance",
        type=float,
...
def main():
    ...
    visualizer.set_headers(
        ["Name", "Origin", "Inverted"] + [f"Step {i:02d}" for i in range(1, step + 1)]
    )
    ...
    for img_idx in tqdm(range(num_images), leave=False):
        output_images = generator.easy_synthesize(
            codes[img_idx], latent_space_type="wp"
        )["image"]
